...
const utils = require('@huyk-utils/utils');
const atomEvent = require('@huyk-utils/atom-event');
...
class ImmutableRequestApiDef {
  static create(store) {
    ...
    Reflect.set(n, kv_m$1, Object.assign({}, utils.cloneData(store) || { url: "" }));
    return n;
  }
  constructor() {
  }
...
const RequestMethod = zod.z.enum([
  // 'get',
  "GET",
  // 'delete',
  "DELETE",
...
const RequestHeaders = zod.z.record(zod.z.string().min(1), zod.z.string());
const RequestBody = zod.z.any();
const ResponseType = zod.z.enum([
  "arraybuffer",
  "blob",
  "document",
  /**
...
const Request = zod.z.object({
   ...
   * 自定义数据
   */
  custom: zod.z.record(zod.z.string(), zod.z.any()).default({})
}).passthrough();
const ResponseHeaders = zod.z.record(zod.z.string().min(1), zod.z.string());
const ResponseCookies = zod.z.array(zod.z.string());
const BaseResponse = zod.z.object({
  data: zod.z.any().default(null),
  /**
   * http请求状态码
   */
  status: zod.z.number().int(),
  statusText: zod.z.string().default(""),
  headers: ResponseHeaders.default({}),
  cookies: ResponseCookies.default([])
});
const Response = BaseResponse.passthrough();
const RequestApiSchema = zod.z.object({
  /**
   * 基路径
   */
  ...
  method: RequestMethod.default("GET"),
  responseType: ResponseType.default("json"),
  headers: zod.z.optional(zod.z.function().args(
    zod.z.any()
  ).returns(zod.z.promise(RequestHeaders).or(RequestHeaders))),
  /**
   * 请求数据模型
   */
   ...
   * ```
   */
  responseStruct: zod.z.unknown().optional(),
  /**
   * 自定义数据
   */
  custom: zod.z.record(zod.z.string(), zod.z.any()).default({})
...
const RequestOptions = zod.z.object({
  /**
   * @default false
   */
  loading: zod.z.boolean().default(false),
  loadingText: zod.z.string().default(""),
  /**
   ...
   * @default 200
   */
  loadingDelay: zod.z.number().int().default(200),
  /**
   * 超时时间（毫秒）
   * @default 20000
   */
  timeout: zod.z.number().default(2e4),
  headers: RequestHeaders.default({}),
  /**
   ...
   * @default (:[a-zA-Z_]\\w*)
   */
  pathParamsPattern: zod.z.string().default(PathParamsPattern),
  /**
   * 自定义数据
   */
  custom: zod.z.record(zod.z.string(), zod.z.any()).default({})
...
async function send(observer, data, opts) {
  ...
  try {
    ...
    emitEffects(observer, "prepare", interceptorParam, () => utils.omit(interceptorParam, ["apiDef"]));
...
class ImmutableRequestObserver {
  ...
  static create(kv, effector, interceptor) {
    ...
    const kvm = Object.assign({}, { limit: 1, race: true }, utils.cloneData(kv));
    Reflect.set(n, kv_m, kvm);
    Reflect.set(n, lmt, new utils.AsyncExecutionLimiter({ race: kvm.race, limit: kvm.limit }));
    Reflect.set(n, ef, effector || new atomEvent.AtomEvent());
    Reflect.set(n, icts, interceptor || new atomEvent.AtomEventPool(["prepare", "pending", "resolved", "rejected"]));
    return n;
  }
  /**
   * @public
  ...
  static cloneEffector(source, target, disableInheritListeners) {
    target[ef] = disableInheritListeners ? new atomEvent.AtomEvent() : source[ef].clone();
  }
  /**
   * @public
   */
...
exports.ImmutableRequestApiDef = ImmutableRequestApiDef;
exports.ImmutableRequestObserver = ImmutableRequestObserver;
exports.Request = Request;
exports.RequestApiSchema = RequestApiSchema;
exports.RequestBody = RequestBody;
exports.RequestException = RequestException;
exports.RequestExceptionTypeEnum = RequestExceptionTypeEnum;
...
exports.defineRequestApi = defineRequestApi;
exports.request = request;
