...
const formatInputArgs = (args, mapData) => args.map((item) => typeof item === "string" ? utils.formatString(item, mapData) : item);
...
const _BaseLogger = class _BaseLogger {
  constructor() {
    __publicField(this, _a, utils.ImmutableStore.create({
      label: "BaseLogger",
